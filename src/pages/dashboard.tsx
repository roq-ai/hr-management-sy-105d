...
import { LessonListPage as ShowCaseList } from 'pages/lessons';

function DashboardPage() {
  const { session } = useSession();

  const exerciseStat = useSWR<PaginatedInterface<ExerciseInterface>>(
    () => `/exercises`,
    () => getExercises({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Exercises',
      result: exerciseStat?.data?.totalCount,
      isLoading: exerciseStat.isLoading,
    },
    {
      title: 'Lessons',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`lessons`} passHref legacyBehavior>
                  <Button
