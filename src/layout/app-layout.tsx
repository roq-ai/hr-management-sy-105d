...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<WebsiteInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/websites` : null),
    () => getWebsites({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Websites',
      path: isTenantUser ? `/websites/view/${data?.[0]?.id}` : '/websites',
      entity: 'website',
      service: AccessServiceEnum.PROJECT,
      icon: FiGlobe,
    },
    { name: 'Exercises', path: '/exercises', entity: 'exercise', service: AccessServiceEnum.PROJECT, icon: FiEdit2 },
